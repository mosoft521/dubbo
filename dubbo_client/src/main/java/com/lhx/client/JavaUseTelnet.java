...
/**
 * Created by Lifeix on 14-11-25.
 */
public class JavaUseTelnet {
     ...
     * @param prompt    结果结束标识
     */
    public JavaUseTelnet(String termtype,String prompt){
        telnet = new TelnetClient(termtype);
        setPrompt(prompt);
    }

    public JavaUseTelnet(String termtype){
        telnet = new TelnetClient(termtype);
    }

    public JavaUseTelnet(){
    ...
    public static void main(String[] args) {
        JavaUseTelnet telnet = new JavaUseTelnet("VT220",">"); //Windows,用VT220,否则会乱码
        telnet.login("192.168.2.155", 20881);
        String rs = telnet.sendCommand("invoke IdentityService.incrementAndGet(\"\")");
        try {
